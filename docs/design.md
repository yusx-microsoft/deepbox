...
server: 校验 token
        ...
        └─ 有效 → 解析 Devbox D
                   ...
                   conn = DevboxConn(devbox_id=D.id, agent_ids={…})
                   把所有 host 的 agent presence 置为 online
                   touch devbox.last_seen_at
                   下发 hello { devbox_id, agent_ids, protocol_version }
```
断开时：把该 Devbox 的所有 agent 置为 offline（除非同一 Devbox 还有别的连接）。
...
| `POST /api/devboxes/:id/runtimes` `{capabilities}` | connector 上报本机可用 CLI |
...
用户自启进程。启动：
```
python -m connector --server-url http://localhost:8000 --token hpc_box_...
